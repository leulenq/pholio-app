  ...
  asyncHandler(async (req, res) => {
    ...
    for (const file of req.files) {
      try {
        ...
        console.log("[Media Upload] Inserted image:", {
          imageId,
        ...
        if (!hasValidHero && !heroSet && uploadedImages.length === 0) {
          console.log(
            "[Media Upload] Setting initial primary image:",
            processed.path,
          );
          await knex("images")
            .where({ id: imageId })
    ...
    if (uploadedImages.length > 0) {
      const totalImages =
        Number(countResult?.total || 0) + uploadedImages.length;
...
router.put(
  ...
  asyncHandler(async (req, res) => {
    ...
    if (!image)
      return res.status(404).json({ success: false, message: "Image found" });
