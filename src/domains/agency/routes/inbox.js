...
const {
  ...
  createDismissal,
  removeDismissal,
} = require("../services/agency-dismissals");
const {
  writeQueryLog,
...
/**
 * A timestamp as an ISO instant, whichever dialect produced it.
 *
 * PostgreSQL hands back a `Date`; SQLite hands back "YYYY-MM-DD HH:MM:SS" with
 * no zone, which is UTC but which `new Date()` would read as server-local and
 * silently shift. Same idiom as `toIsoTimestamp` in
 * `talent/services/application-drafts.js`.
 */
function discoverIsoTimestamp(value) {
...
/**
 * Whether THIS agency has touched this talent before, and nothing about any
 * other agency (design §3, item 7: "approaching the same person twice is the
 * embarrassment this prevents").
 *
 * Both reads are scoped to the session agency. `applications` is ordered
 * newest-first because a talent may have applied to the same agency across
 * separate calls, and the state a scout needs is the current one.
 */
async function loadDiscoverPriorContact(db, { agencyId, profileId }) {
  const empty = {
    invited_at: null,
    applied_at: null,
    application_status: null,
  };
  if (!agencyId || !profileId) return empty;

  const [invitation, application] = await Promise.all([
    findInvitation(db, { agencyId, profileId }),
    db("applications")
      .where({ profile_id: profileId, agency_id: agencyId })
      .orderBy("created_at", "desc")
      .first()
  ...
  return {
    invited_at: discoverIsoTimestamp(invitation?.created_at),
    applied_at: discoverIsoTimestamp(application?.created_at),
...
router.get(
  ...
  async (req, res, next) => {
    try {
      ...
      // Internal analytics only. A scout opening a search result is a read,
      // and this is the record of it that the product itself uses.
      recordProfileEvent({
        profile,
      ...
      //
      // This endpoint used to fire `notifyTalentAgencyProfileView`, which was
      // harmless only because nothing called it: the expanded Scout view read
      // its data from the grid card and never fetched a preview. Now that the
      // expanded view loads the profile, a scout flipping through a result set
      // would tell every talent they passed over that "an agency viewed your
      // profile" — a claim of interest generated by scrolling.
      //
      // Scout sits before the application lifecycle (design §1): the agency
      // owes the talent nothing at a read, and the talent learns nothing from
      // one. So the notification moved to the deliberate act that follows a
      // read worth acting on — the invitation, below. It is the only producer
      // of `agency_profile_view`, so the talent's interest signal survives the
      // move; it now means an agency chose to approach rather than glanced.
      //
      // If a surface ever needs notify-on-read again, it belongs behind a
      // talent-visible setting, not on a search result.
      ...
      //
      // Representations are loaded for the same reason the grid loads them
      // (`attachImagesAndInvites` in discover-search.js): without them
      // `deriveRepresentationStatus` sees only `seeking_representation` and can
      // never return `represented` or `exclusive_elsewhere` — the one fact that
      // would stop a scout approaching someone with a mother agency.
      const [lanesByProfile, representationsByProfile, priorContact, dismissal] =
        await Promise.all([
          loadLanesByProfile(knex, [profileId]),
          loadTalentRepresentationsForProfiles([profileId], { db: knex }),
          loadDiscoverPriorContact(knex, { agencyId, profileId }),
          findDismissal(knex, { agencyId, profileId }),
        ]);
      ...
      const dto = buildAgencyDiscoveryDTO(profile, {
        ...
        representations: representationsByProfile.get(profileId) || [],
      });

      // Set on the response, never added to AGENCY_DISCOVERY_FIELDS — same
      // treatment as `is_invited` on the grid. `updated_at` is a workflow
      // column, not a profile property the allowlist should carry; here it is
      // the freshness fact the surface states in words ("Updated 3 weeks ago").
      dto.profile_updated_at = discoverIsoTimestamp(profile.updated_at);
      dto.contact = priorContact;
      // Agency-private view state. Never notified, never exported to the talent.
      dto.dismissed = Boolean(dismissal);

      return res.json({ success: true, profile: dto });
    } catch (error) {
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      // The talent-facing signal, moved here from the preview read (see the
      // note in that handler). It says what actually happened: this agency
      // asked. The old copy said "viewed your profile", which was true of a
      // glance and false of an invitation — and the talent already has the
      // invitation email, so a second message describing it as a look would
      // contradict the first.
      // Best-effort: a failed notification never fails a recorded invitation.
      if (profile.user_id) {
        try {
          await notifyTalentAgencyInvitation({
            userId: profile.user_id,
            agencyId,
            agencyName,
          });
        } catch (notifyError) {
          console.error("[Discover Invite] Notification failed:", notifyError);
        }
      }
...
/**
 * "Not for us" — POST to dismiss, DELETE to undo.
 *
 * The private half of the Scout bar. It drops the profile out of this agency's
 * future Discover results and changes nothing else: no notification, no
 * activity, no application, no record the talent can ever read. The talent was
 * never told they were found, and is not told they were passed over.
 *
 * Both verbs are idempotent, and both resolve the target the same way the
 * sibling preview and invite handlers do — a discoverable profile this agency
 * is allowed to see — so an agency cannot use dismissal to probe for the
 * existence of a profile Discover would not show it.
 */
function resolveDismissTarget(handler) {
  return async (req, res, next) => {
    try {
      ...
      return await handler({ req, res, agencyId, profileId });
    } catch (error) {
      console.error("[API/Agency/Discover Dismiss] Error:", error);
      return next(error);
    }
  };
}

// The store arrives with a migration; a deploy that lands first must say so
// rather than reporting a dismissal it did not record.
const DISMISS_UNAVAILABLE = {
  error: "dismiss_unavailable",
  message: "Pholio is finishing an update. Please try again shortly.",
};

// POST /api/agency/discover/:profileId/dismiss
router.post(
  "/api/agency/discover/:profileId/dismiss",
  discoverLimiter,
  requireRole("AGENCY"),
  resolveDismissTarget(async ({ res, agencyId, profileId }) => {
    const stored = await createDismissal(knex, {
      agencyId,
      profileId,
      id: require("crypto").randomUUID(),
    });
    if (!stored) return res.status(503).json(DISMISS_UNAVAILABLE);
    return res.json({ success: true, dismissed: true });
  }),
);

// DELETE /api/agency/discover/:profileId/dismiss
router.delete(
  "/api/agency/discover/:profileId/dismiss",
  discoverLimiter,
  requireRole("AGENCY"),
  resolveDismissTarget(async ({ res, agencyId, profileId }) => {
    const removed = await removeDismissal(knex, { agencyId, profileId });
    if (!removed) return res.status(503).json(DISMISS_UNAVAILABLE);
