...
async function attachLocals(req, res, next) {
  ...
  if (req.session && req.session.userId) {
    try {
        ...
        if (user.role === "TALENT") {
          try {
            ...
            try {
              ...
              console.error(
                "[attachLocals] Error loading subscription:",
            ...
            console.error(
              "[attachLocals] Error loading profile:",
      ...
      if (
        ...
        console.error(
          "[attachLocals] Database connection error:",
          error.message,
        );
        // For dashboard and PDF routes, let the error propagate so it can be handled properly
        if (
          req.path &&
          (req.path.startsWith("/dashboard") || req.path.startsWith("/pdf"))
        ) {
          return next(error);
        }
    ...
    const {
      extractIdToken,
    } = require("../../domains/auth/middleware/firebase-auth");
    const {
      verifyIdToken,
    ...
    if (idToken) {
      try {
        ...
        console.log(
          "[attachLocals] Firebase token verification failed (non-critical):",
          error.message,
        );
      }
    }
  ...
  if (
    ...
    !res.locals.firebaseConfig.authDomain ||
    !res.locals.firebaseConfig.projectId
  ) {
    console.warn(
      "[Firebase Config] Missing client-side Firebase configuration:",
    );
    console.warn("[Firebase Config]", {
      ...
      storageBucket: res.locals.firebaseConfig.storageBucket
        ? "SET"
        : "MISSING",
      messagingSenderId: res.locals.firebaseConfig.messagingSenderId
        ? "SET"
        : "MISSING",
      ...
      envVars: {
        ...
        VITE_FIREBASE_STORAGE_BUCKET: process.env.VITE_FIREBASE_STORAGE_BUCKET
          ? "SET"
          : "MISSING",
        VITE_FIREBASE_MESSAGING_SENDER_ID: process.env
