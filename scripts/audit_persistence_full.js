...
const { talentProfileUpdateSchema } = require("../src/shared/lib/validation");
...
const {
  ...
  convertLbsToKg,
  toFeetInches,
} = require("../src/domains/talent/services/profile-helpers");
...
async function runAudit() {
  ...
  const testPayload = {
    ...
    previous_representations: [
      { agency_name: "Old Agency", reason_leaving: "Contract ended" },
    ],
    experience_details: [
      { role: "Lead", production: "Test Movie", year: "2023" },
    ],
  ...
  const mapField = (field, dbField = field) => {
    if (data[field] !== undefined)
      updateData[dbField] = data[field] === "" ? null : data[field];
  };
  ...
  if (data.bio !== undefined) {
    ...
    updateData.bio_curated = data.bio
      ? curateBio(data.bio, data.first_name || "Test", data.last_name || "User")
      : "";
  }
  ...
  console.log(
    `✅ Mapped ${Object.keys(updateData).length} fields to database columns.`,
  );

  // 5. Gap Analysis (Payload vs DB)
  const mappedKeys = Object.keys(updateData);
  const missingInDB = mappedKeys.filter(
    (key) => key !== "updated_at" && !dbColumns.includes(key),
  );
  ...
  const unmappedPayloadKeys = payloadKeys.filter((k) => {
    ...
    if (
      ...
      (updateData.weight_kg !== undefined ||
        updateData.weight_lbs !== undefined)
    )
      return false;
    return true;
  });
