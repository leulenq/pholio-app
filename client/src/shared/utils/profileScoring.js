...
export const calculateProfileStrength = (data) => {
  ...
  const hasHeadshot = book.hasHeadshot || isPresent(data.primary_photo_id) || isPresent(data.hero_image_path);
  const hasFullBody = book.hasFullBody;

  if (hasHeadshot) requiredScore += 8;
  else pushMissing({ key: 'photo_headshot', label: 'Headshot', why: 'A clean, natural headshot is the first image on every agency digitals set.', impact: 'Critical', link: '/dashboard/talent/media', points: 8, tier: 'Required' });

  if (sensitiveRequired) {
    if (hasFullBody) requiredScore += 12;
    else pushMissing({ key: 'photo_full_body', label: 'Full-Body Photo', why: 'Agents need a head-to-toe frame to verify proportions and stance.', impact: 'Critical', link: '/dashboard/talent/media', points: 12, tier: 'Required' });
  }

  const bioSource = data.bio ?? data.bio_raw ?? '';
  const hasBio = String(bioSource).trim().length > 50;
  if (hasBio) improveScore += 6;
  else pushMissing({ key: 'bio', label: 'Professional Bio', why: 'A short bio gives context beyond stats — training, market, and personality.', impact: 'Medium', link: '/dashboard/talent/profile?tab=identity', points: 6, tier: 'Improve' });

  const hasLook = isPresent(data.eye_color) && isPresent(data.hair_color);
  if (hasLook) improveScore += 5;
  else pushMissing({ key: 'look', label: 'Eye & Hair Color', why: 'Hair and eye color belong on every comp-card stats block.', impact: 'Medium', link: '/dashboard/talent/profile?tab=appearance', points: 5, tier: 'Improve' });

  const hasShoe = isPresent(data.shoe_size);
  if (hasShoe) improveScore += 4;
  else pushMissing({ key: 'shoe', label: 'Shoe Size', why: 'Footwear sizing is standard on agency stats sheets and castings.', impact: 'Low', link: '/dashboard/talent/profile?tab=appearance', points: 4, tier: 'Improve' });

  const hasWeight = isPresent(data.weight_kg) && Number(data.weight_kg) > 0;
  if (sensitiveRequired) {
    if (hasWeight) improveScore += 2;
    else pushMissing({ key: 'weight', label: 'Weight', why: 'Some markets list weight alongside measurements for fit checks.', impact: 'Low', link: '/dashboard/talent/profile?tab=appearance', points: 2, tier: 'Improve' });
  }
  ...
  else pushMissing({ key: 'skin', label: 'Skin Tone & Markings', why: 'Visible tattoos, piercings, and skin tone prevent set-day surprises.', impact: 'Medium', link: '/dashboard/talent/profile?tab=appearance', points: 3, tier: 'Improve' });

  const hasStatus = isPresent(data.work_status);
  if (hasStatus) improveScore += 4;
  else pushMissing({ key: 'status', label: 'Work Status', why: 'Availability signals whether you can take bookings now.', impact: 'Medium', link: '/dashboard/talent/profile?tab=roles', points: 4, tier: 'Improve' });

  const hasExpLevel = isPresent(data.experience_level);
  if (hasExpLevel) improveScore += 3;
  else pushMissing({ key: 'exp', label: 'Experience Level', why: 'New faces and working talent are pitched differently to clients.', impact: 'Low', link: '/dashboard/talent/profile?tab=credits', points: 3, tier: 'Improve' });
  ...
  else pushMissing({ key: 'training', label: 'Training & Specialties', why: 'Skills and languages show bookers what you can do once the brief fits.', impact: 'Medium', link: '/dashboard/talent/profile?tab=training', points: 4, tier: 'Improve' });

  const hasSocial = isPresent(data.instagram_handle) || isPresent(data.portfolio_url);
  if (hasSocial) improveScore += 6;
  else pushMissing({ key: 'social', label: 'Social or Portfolio Link', why: 'Instagram and portfolio links are how scouts verify your current look.', impact: 'Medium', link: '/dashboard/talent/profile?tab=socials', points: 6, tier: 'Improve' });

  const hasContact = isPresent(data.email) && isPresent(data.phone);
  if (hasContact) improveScore += 3;
  else pushMissing({ key: 'contact', label: 'Email & Phone', why: 'Agencies need direct contact details to follow up on submissions.', impact: 'Medium', link: '/dashboard/talent/settings', points: 3, tier: 'Improve' });
...
export const getStrengthUI = (score, isRequiredComplete = false) => {
  ...
  if (score < 85) {
    return {
      ...
      color: '#C9A55A',
      message: 'Essentials complete. Add details to strengthen your profile.',
      status: 'improvement',
    };
  }
  ...
  if (score < 100) {
    return {
      label: 'Submission ready',
      color: '#2D8A56',
      message: 'Ready for agency review.',
      status: 'improvement',
    };
  }
