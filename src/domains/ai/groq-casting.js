 ...
 *
 * Step 1 — Scout (llama-4-scout, vision)
...
async function runDirector(scoutResult, stats) {
  const completion = await getGroq().chat.completions.create({
    ...
    messages: [
      { role: "user", content: buildDirectorPrompt(scoutResult, stats) },
    ],
    temperature: 0.4,
    max_completion_tokens: 512,
    response_format: { type: "json_object" },
...
function fallbackDirector(scoutResult, stats) {
  ...
  if (height_cm >= minRunway) {
    runway += 20;
    editorial += 15;
  } else if (height_cm >= minRunway - 5) {
    editorial += 10;
    commercial += 5;
  } else {
    commercial += 10;
  ...
  if (/commercial|friendly|approachable|girl.next.door/.test(tags))
    commercial += 15;
  if (/athletic|sporty|fit/.test(tags)) lifestyle += 15;
  if (/classic|elegant|refined/.test(tags)) runway += 10;

  // Build signal
  if (/slender|lean/.test(build)) {
    runway += 10;
    editorial += 5;
  }
  if (/athletic/.test(build)) {
    lifestyle += 10;
    commercial += 5;
  }
  if (/curvy/.test(build)) {
...
function blendArchetypeScores(
  ...
  return {
    runway: clamp(
      directorScores.runway * directorWeight +
        vectorScores.runway * vectorWeight,
    ),
    editorial: clamp(
      directorScores.editorial * directorWeight +
        vectorScores.editorial * vectorWeight,
    ),
    commercial: clamp(
      directorScores.commercial * directorWeight +
        vectorScores.commercial * vectorWeight,
    ),
    lifestyle: clamp(
      directorScores.lifestyle * directorWeight +
        vectorScores.lifestyle * vectorWeight,
    ),
  };
}
...
async function generateArchetype(knex, profileId, photoStorageKey, stats = {}) {
  ...
  try {
    ...
    console.error("[groq-casting] Scout (vision) failed:", err.message);
  ...
  try {
    ...
    if (scoutText && knex && profileId) {
      ...
      await reindexDiscoverProfile(knex, profileId, { scout: scoutResult });
    }
  } catch (embErr) {
    console.warn(
      "[groq-casting] Image embedding failed (non-blocking):",
      embErr.message,
    );
  }
  ...
  try {
    ...
    console.error(
      "[groq-casting] Director (openai/gpt-oss-20b) failed, using fallback:",
      err.message,
    );
  ...
  const primaryArchetype =
    ARCHETYPE_LABELS[top[0]] ||
    directorResult.primary_archetype ||
    "Versatile Talent";
