...
const SPECIALTY_KEYWORDS = {
  [PROFILE_DIVISIONS.COMMERCIAL_LIFESTYLE]: ['commercial', 'lifestyle', 'catalog', 'e-commerce', 'ecommerce'],
  ...
  [PROFILE_DIVISIONS.FIT_SHOWROOM]: ['fit', 'showroom'],
};
...
const DIVISION_READINESS_CONFIG = {
...
export function resolveTalentDivision(profile = null) {
  ...
  if (
    includesAny(specialties, SPECIALTY_KEYWORDS[PROFILE_DIVISIONS.TALENT_PERFORMANCE]) ||
    includesAny([experience], EXPERIENCE_HINTS_PERFORMANCE)
  ) {
    return PROFILE_DIVISIONS.TALENT_PERFORMANCE;
  }

  if (includesAny(specialties, SPECIALTY_KEYWORDS[PROFILE_DIVISIONS.FIT_SHOWROOM])) {
    return PROFILE_DIVISIONS.FIT_SHOWROOM;
  }

  if (includesAny(specialties, SPECIALTY_KEYWORDS[PROFILE_DIVISIONS.COMMERCIAL_LIFESTYLE])) {
    return PROFILE_DIVISIONS.COMMERCIAL_LIFESTYLE;
  }

  return PROFILE_DIVISIONS.FASHION_EDITORIAL;
}

export function getDivisionReadinessConfig(division) {
  return DIVISION_READINESS_CONFIG[division] || DIVISION_READINESS_CONFIG[PROFILE_DIVISIONS.FASHION_EDITORIAL];
}

export { PROFILE_DIVISIONS };
