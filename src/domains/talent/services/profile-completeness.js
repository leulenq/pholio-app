...
async function computeProfileCompleteness(profileId) {
  ...
  const profilePhotos = await knex("profile_photos")
  ...
  const optional = {
    ...
    social:
      hasValue(profile.instagram_handle) ||
      hasValue(profile.twitter_handle) ||
      hasValue(profile.tiktok_handle),
    languages:
      profile.languages &&
    ...
    experience:
      hasValue(profile.experience_level) ||
      hasValue(profile.experience_details) ||
      hasValue(profile.training),
    availability:
      hasValue(profile.availability_schedule) ||
      profile.availability_travel !== null,
    specialties:
      profile.specialties &&
