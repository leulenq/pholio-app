...
| 2 | **Wilhelmina** | Accessible major | A global name with a real Curve division and broad boards — though deep research falsified the "curve/fitness/non-binary, 16 and up" welcome copy the selection originally relied on (it appears nowhere on the live site; the form is binary WOMEN/MEN and hard-blocks under-18 DOBs while displaying guardian language). It remains in the set as a major the cohort applies to regardless, and as the sharpest evidence that marketing-copy claims must never enter the registry unverified. NYDOL cert 26-69UG6-LSFW (late registrant). Market-selector form stress-tests multi-market routing. |
| 3 | **Elite Model Management (NA)** | Flagship reference major | The brand every aspirant knows — and the highest-surprise-density application in the set, though not for the reasons folklore claimed: deep research found NO published stat floors (the circulating figures are third-party), HEIC un-blocked but un-warned (server-side failure mode), a promised under-18 guardian step that does not exist in the form DOM, internally contradictory age wording on one page, and "female identified persons" scoping with no NY men's board in the sitemap. NYDOL cert 26-69YIX-LSFW (the DOL certificate PDF is hosted on Elite's own site). Multiple Elite routes (NA vs the separate Elite Model Look contest entity) are themselves a finding to model. |
| 4 | **Ford Models** | Reference major; multi-provider routing archetype | Second-most-recognized brand in the pool. Deep research corrected the internal premise: the canonical NYC form is hosted on selectroom.app (Snapcast serves only the Paris route), so one agency spans two provider platforms with structurally different forms — the set's densest test of per-city routing, provider forms, and measurement blocks. NYDOL cert 26-69V5K-LSFW (late — post-deadline registration). |
| 5 | **State Management** | Commercial/fit/kids breadth; provider-form archetype B | Explicitly commercial/e-commerce/fit segments serve the non-runway majority of the cohort. Deep research found State runs TWO parallel, unlinked application channels — its own native form and a Snapcast form — with different photo formats, size caps, and legal regimes; the strongest single case for multi-channel modeling in the registry, and still a second Snapcast instance for the per-platform verification thesis. NYDOL cert 26-66CVV-LSFW. Three inconsistent kids-email channels are a minors-routing data point. |
