...
} = require("../domains/auth/middleware/session-validator");
const {
...
const groq = new Groq({
  apiKey:
    ...
    "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",
});
  ...
  async (req, res, next) => {
    try {
      ...
      if (req.session.userId || req.user?.id || req.body.userId) {
        ...
        try {
          ...
          if (userData && userData.role === "TALENT") {
            profileData = await knex("profiles")
          ...
          if (userData && userData.email) {
            const hasName =
              profileData?.first_name || req.session.onboardingData?.firstName;
            const hasEmail = userData.email;
            const hasLocation =
              profileData?.city || req.session.onboardingData?.city;
      ...
      if (onboardingData.height_cm && currentStage === 3) {
        ...
        const apparentGender =
          onboardingData.visualIntel?.analysis?.visualEstimates?.apparentGender;
        const bodyType =
          onboardingData.visualIntel?.analysis?.visualEstimates?.bodyType;
        ...
        if (bodyType === "endomorph") {
          baseWeight = Math.round(baseWeight * 1.1); // +10% for endomorph
        } else if (bodyType === "ectomorph") {
          baseWeight = Math.round(baseWeight * 0.9); // -10% for ectomorph
        }
        ...
        projections = {
          ...
          hips: Math.round(hIn * (isMale ? 0.53 : 0.5)),
        };
      ...
      try {
        ...
        if (currentStage === 3) {
          console.log(
            "[Chat API] Stage 3 response from Maverick:",
            JSON.stringify(responseData, null, 2),
          );
          if (!responseData.tool_trigger) {
            console.warn(
              "[Chat API] Stage 3 response missing tool_trigger! Response:",
      ...
      if (
        typeof responseData.stage === "number" &&
        responseData.stage >= 0 &&
        responseData.stage <= 7
      ) {
      ...
      if (currentStage === 3 && !req.session.onboardingData.height_cm) {
        ...
        if (heightMatch) {
          ...
          console.log(
            "[Chat API] Extracted height_cm from message:",
            req.session.onboardingData.height_cm,
          );
        }
      }
      ...
      if (responseData.action === "complete" && currentStage === 7) {
        ...
        await triggerLibrarianSynthesis(
          req.session.onboardingData,
          userId,
          req,
        );
      ...
      if (req.session.currentStage === 3 && !toolTrigger) {
        console.warn(
          "[Chat API] Stage 3 response without tool_trigger - Maverick should have provided one",
        );
      }
...
Respond in JSON:
{
  ...
  "isUnicorn": true/false
    ...
    if (userId && req.session.userId) {
      await createProfileFromSynthesis(
        onboardingData,
        synthesis,
        req.session.userId,
      );
...
async function createProfileFromSynthesis(onboardingData, synthesis, userId) {
  try {
    ...
    if (existingProfile) {
      ...
      const verifiedCity =
        onboardingData.ipGeolocation?.city ||
        onboardingData.verifiedLocationIntel?.ip_verified?.city;
      const verifiedCountry =
        onboardingData.ipGeolocation?.country ||
        onboardingData.verifiedLocationIntel?.ip_verified?.country;
      const verifiedRegion =
        onboardingData.ipGeolocation?.region ||
        onboardingData.verifiedLocationIntel?.ip_verified?.region;
      ...
      const updateData = {
        ...
        ip_timezone:
          onboardingData.ipGeolocation?.timezone ||
          sqlData.ip_timezone ||
          undefined,
        verified_location_intel: onboardingData.verifiedLocationIntel
          ? JSON.stringify(onboardingData.verifiedLocationIntel)
          : sqlData.verified_location_intel || undefined,

        // Google-verified fields (from OAuth, not conversational)
        google_birthday:
          onboardingData.googleBirthday || sqlData.google_birthday || undefined,
        google_gender:
          onboardingData.googleGender || sqlData.google_gender || undefined,
        google_phone:
          onboardingData.googlePhone || sqlData.google_phone || undefined,
        google_addresses: onboardingData.googleAddresses
          ? JSON.stringify(onboardingData.googleAddresses)
          : sqlData.google_addresses || undefined,
        google_organization:
          onboardingData.googleOrganization ||
          sqlData.google_organization ||
          undefined,
        ...
        specialties: sqlData.specialties
          ? stringifyJsonField(sqlData.specialties)
          : undefined,
        experience_details: sqlData.experience_details
          ? stringifyJsonField(sqlData.experience_details)
          : undefined,
        languages: sqlData.languages
          ? stringifyJsonField(sqlData.languages)
          : undefined,
        comfort_levels: sqlData.comfort_levels
          ? stringifyJsonField(sqlData.comfort_levels)
          : undefined,
        previous_representations: sqlData.previous_representations
          ? stringifyJsonField(sqlData.previous_representations)
          : undefined,
      ...
      if (
        synthesis.vectorSummary &&
        Array.isArray(synthesis.vectorSummary) &&
        synthesis.vectorSummary.length === 384
      ) {
        updateData.vector_summary = synthesis.vectorSummary;
      } else if (
        synthesis.vectorSummary &&
        typeof synthesis.vectorSummary === "string"
      ) {
        ...
        console.log(
          "[Librarian] Skipping vector_summary - text string provided, needs embedding conversion",
        );
      }
      ...
      console.log(
        "[Librarian] Updated existing profile (sqlData as single source of truth):",
        {
    ...
    const bioCurated =
      sqlData.bio_curated ||
      (bioRaw ? curateBio(bioRaw, firstName, lastName) : "");
    ...
    const profileData = {
      ...
      ip_timezone:
        onboardingData.ipGeolocation?.timezone ||
        onboardingData.verifiedLocationIntel?.ip_verified?.timezone ||
        sqlData.ip_timezone ||
        null,
      verified_location_intel: onboardingData.verifiedLocationIntel
        ? JSON.stringify(onboardingData.verifiedLocationIntel)
        : sqlData.verified_location_intel || null,

      // Google-verified fields (from OAuth, not conversational)
      google_birthday:
        onboardingData.googleBirthday || sqlData.google_birthday || null,
      google_gender:
        onboardingData.googleGender || sqlData.google_gender || null,
      google_phone: onboardingData.googlePhone || sqlData.google_phone || null,
      google_addresses: onboardingData.googleAddresses
        ? JSON.stringify(onboardingData.googleAddresses)
        : sqlData.google_addresses || null,
      google_organization:
        onboardingData.googleOrganization ||
        sqlData.google_organization ||
        null,
    ...
    console.log("[Librarian] Creating profile with onboarding data:", {
      ...
      hasMeasurements: Boolean(
        profileData.bust && profileData.waist && profileData.hips,
      ),
      hasComfortLevels: Boolean(profileData.comfort_levels),
      hasAvailability: Boolean(
        profileData.availability_schedule ||
        profileData.availability_travel !== null,
      ),
      hasExperience: Boolean(
        profileData.experience_level || profileData.training,
      ),
      fromOnboardingData: {
    ...
    console.log("[Librarian] Created profile:", {
      profileId,
      userId,
      isUnicorn,
      vibeScore,
...
function calculateModelScore(profile, images = [], onboardingData = {}) {
  ...
  if (profile.height_cm && profile.bust && profile.waist && profile.hips) {
    ...
    const avgScore =
      measurementScores.reduce((a, b) => a + b, 0) / measurementScores.length;
  ...
  if (scoutAnalysis?.analysis) {
    ...
    const facialSymmetry =
      scoutData.facialSymmetryScore || scoutData.facialSymmetry || 0;
    ...
    if (imageCount >= 3)
      photoScore += 5; // +5 points for 3+ images
  ...
  const totalScore = Math.min(
    completenessScore + proportionsScore + photoScore,
    100,
  );
...
function generateMarketTags(profile, scoutAnalysis = null) {
  ...
  const hasRunwayProportions =
    profile.height_cm >= 175 &&
    profile.waist &&
    parseFloat(profile.waist) <= 26 &&
    profile.bust &&
    parseFloat(profile.bust) >= 32 &&
    parseFloat(profile.bust) <= 34;
  ...
  const hasCommercialProportions =
    profile.bust &&
    profile.waist &&
    profile.hips &&
    parseFloat(profile.bust) >= 30 &&
    parseFloat(profile.bust) <= 38 &&
    parseFloat(profile.waist) >= 24 &&
    parseFloat(profile.waist) <= 30;
...
function generateActionPlan(profile, images = []) {
  ...
  if (imageCount < 6) {
    actions.push(
      `Add ${6 - imageCount} more photos to complete your portfolio`,
    );
  }
...
router.post(
  ...
  async (req, res, next) => {
    try {
      ...
      if (currentStage < 4) {
        console.log(
          "[Reveal] Stage too early - current stage:",
          currentStage,
          "minimum required: 4",
        );
      ...
      if (!req.session.onboardingData?._synthesis) {
        console.log(
          "[Reveal] Synthesis not found, triggering Librarian synthesis...",
        );
        try {
          await triggerLibrarianSynthesis(
            req.session.onboardingData,
      ...
      if (!profile && req.session.onboardingData) {
        profile = {
          ...
          emergency_contact_name:
            req.session.onboardingData.emergency_contact_name || null,
          emergency_contact_phone:
            req.session.onboardingData.emergency_contact_phone || null,
          partner_agency_id:
            req.session.onboardingData.partner_agency_id || null,
        };
      }
      ...
      const scoreData = calculateModelScore(
        profile,
        images,
        req.session.onboardingData,
      );
      ...
      if (images && images.length > 0 && images[0].path) {
        heroImage = images[0].path.startsWith("http")
          ? images[0].path
          : `/uploads/${images[0].path.split("/").pop()}`;
      }
      ...
      return res.json({
        ...
        firstName:
          profile?.first_name ||
          req.session.onboardingData?.firstName ||
          "Talent",
