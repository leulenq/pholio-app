...
export function normalizeLocation(input) {
  ...
  // Exact match in CITIES registry
  const match = CITIES.find((c) => c.label.toLowerCase() === str.toLowerCase() || c.name.toLowerCase() === str.toLowerCase());
  if (match) {
    const regionCode = US_STATE_TO_ST[match.region] || '';
    const countryCode = match.country === 'United States' ? 'US' : '';
    return {
      city: match.name,
      region: match.region,
      regionCode,
  ...
  // Parse string components: "City, ST" or "City, State, Country" or "City, Country"
