/**
 * Image helpers for agency-readiness scoring.
 * Agencies expect digitals: at minimum a clear headshot and a full-body frame.
 */
...
/** Digitals slot: natural/polaroid-style, not styled book work (spec §4.2). */
function isDigitalSlot(img) {
  if (!img) return false;
...
function isSmileHeadshot(img) {
  ...
  const signals = parseAiSignals(img);
  return String(signals.expression || "").toLowerCase() === "smile";
}
...
function analyzeBookReadiness(images = []) {
  const list = Array.isArray(images) ? images : [];
