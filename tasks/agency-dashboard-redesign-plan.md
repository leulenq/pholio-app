...
**Rendered visual pass + T11 completion (this session):** drove the live app (Express :3002 + Vite :5173, seeded agency via dev login, Puppeteer + preinstalled Chromium) across every agency route at 1440/1024/768 — zero console errors, zero API failures, zero horizontal overflow after fixes. Defects found and fixed en route: (1) `/api/agency/applications` returned no application fields — knex silently drops `.select()` args following an array arg, so the Submissions desk rendered empty against 40 real submissions; (2) `/api/agency/reminders` 500 (joined nonexistent `applications.talent_id`); (3) missing `POST /api/dev/bootstrap` route that SessionGates call; (4) SPA hard-crashed with no `VITE_FIREBASE_*` config (`auth/invalid-api-key` at module scope) — now degrades to session-only auth; (5) Overview Top-matches corner MATCH chip (banned) → plain inline text; Discover card chip moved in-flow beside the name; (6) imperial height carry bug (182 cm rendered 5′ 12″) in both `rosterFormat.js` and the shared conversion util; (7) demo seed left signed talent roster-less (`roster_memberships` never seeded; backfill migration runs before seeding on fresh DBs) — seed now syncs through the production service; (8) client lockfile pinned all 496 packages to registry.npmmirror.com, stalling installs outside that mirror's reach — repointed to registry.npmjs.org. Left as shipped by intent: roster division/stage sharp tonal tags (documented approved non-badge treatment), avatar-stack "+N" overflow counts. Known pre-existing red: 6 `agency-rbac` integration tests fail identically before and after this work (fixture drift — `users` fixture lacks `account_status` that `requireActiveAccount()` selects); flagged for a dedicated test-repair pass.

**Test-repair pass (this session):** the flagged agency suites are green — 93/93 across `agency-rbac` (42), `agency-overview` (34), `agency-extras` (4), `agency-analytics` (13). Roots fixed: (1) hand-built test schemas had drifted behind the product — added `users.avatar_url`, the full agency-submission audience columns on `profiles`, application minor-gating columns, and the `roster_memberships` / `talent_commitments` / `minor_agency_consents` / `agency_team_invitations` tables; (2) the roster-detail test asserted the pre-membership response contract (`body.profile`) — updated to `data.{membership,talent}` with a membership fixture; (3) team-add now correctly expects `INVITED` (invitation-based RBAC), not immediate `ACTIVE`; (4) the sqlite3 driver stores JS `Date` bindings/inserts as epoch-ms (and stringifies them to garbage under Jest's sandboxed realm), so ISO strings are now used everywhere: overview fixtures, the five `overview.queries.js` functions that hadn't adopted getPulse's documented ISO convention, and the demo seed's date helpers. That last fix also repaired real dev-data bugs: "closing today" boards now count (was 0 vs a visible TODAY row) and the roster trend shows the true growth curve instead of a flat line.

**Full-suite baseline (this session):** whole backend suite: 2175/2258 passing. The 11 red suites (app, e2e-casting-to-dashboard, dto-security, roster-measured-in-person, onboarding adult-launch-eligibility + security-hardening, overview-backend, stripe billing-disclosure, talent application-drafts/media-bulk/open-call-claims) fail identically at the pre-session commit — pre-existing repo-wide test debt, zero regressions from this branch's work (net −11 failing tests vs. that baseline). A few suites (same-origin-app, representations, profile-save-contract) are order-dependent: they fail only in the full parallel run — shared test.sqlite3 interference between hand-built schemas. Recommend a dedicated repo-wide test-debt pass: same drift classes as the agency repair (hand-built schema drift, Date-vs-ISO storage, stale contracts).
