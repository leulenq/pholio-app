...
2. **Repo boundary:** Terms of Service / Submission Program Notice *pages* live in **`pholio-site`**, not here (root `CLAUDE.md` boundary rule). The ToS needs matching fair-use language: limits may change with notice; anti-abuse enforcement (claim revocation, velocity caps) is at Pholio's discretion; open call links are provided to agencies for their own applicant funnels and may not be resold or syndicated. [counsel]
...
Cinematic arrival screen (§6.1); all §6.2 dashboard surfaces; agency self-serve link management + funnel stats; notifications; `SUBMISSION_PROGRAM_CONTENT` version bump; coordinated ToS/privacy update in `pholio-site`. Pilot exits to GA on healthy metrics.
