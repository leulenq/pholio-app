 ...
 *   represented     — any application with group:'represented' (`represented` only)
 *   in_conversation — any application with group:'advancing' (including an accepted offer)
 ...
 *
 * v1 limitation: a talent who signed with an agency offline (no `represented`
 * application row in Pholio) will read as unrepresented. The structured
 ...
 * @returns {{
 *   state: 'represented'|'in_conversation'|'submitted'|'unrepresented',
 *   label: string,
 *   agency: string|null,
...
export function deriveRepresentationStatus(applications = []) {
  ...
  if (representedApp) {
    return {
      state: 'represented',
      label: representedApp.agency_name ? 'Represented by' : 'Represented',
      agency: representedApp.agency_name || null,
      agencyWebsite: representedApp.agency_website || null,
      action: null,
    };
  }
