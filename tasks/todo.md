...
labelled, correctly scoped, and reflected in downstream product behavior.

## Audit findings (verified against consumers, not assumed)

### Presentation section — DELETE ENTIRELY (all three controls fake)
- [x] `cardLayout` (editorial/classic/minimal) — zero consumers. Comp cards are
      generated by `domains/pdf/composition/composition-director.js`, which derives
      layout from the actual frames. A static template picker contradicts the
      generator. Wrong abstraction.
- [x] `coverImage` (first/latest/featured) — zero consumers. The lead frame is set
      explicitly via `talentApi.setHeroImage` in Media. A second "resting state"
      abstraction is a conflicting source of truth.
- [x] `watermark` — `domains/pdf/routes/pdf.js` sets `watermark: !profile.is_pro`
      on all three render paths. It stamps "Pholio", not the user's handle. It is a
      plan marker, not a preference; the label was wrong twice over.
- [x] Drop `talent_user_settings.display_preferences` (nothing else reads it).

### Notifications — remove fake granularity
- [x] `applicationUpdates` — REAL (`shared/services/notifications.js:342`). Keep.
- [x] `profileViews` — REAL (`notifications.js:371`). Keep.
- [x] `newMessages` — REMOVE. `notifications.js` documents that messages and
      interviews are *deliberately* never gated ("a booker reaching out always
      reaches the bell"). The toggle contradicted the code by design.
- [x] `marketing` ("Product notes") — REMOVE. No sender consults it.
- [x] `emailFrequency` (Immediate/Daily/Weekly digest) — REMOVE. No digest job
      exists anywhere in the codebase.
- [x] `inAppApplications`, `emailNotifications` — REMOVE. Dead fields, never rendered.

### Presence
- [x] `isPublic`, `isDiscoverable` — REAL. Keep.
- [x] `showContact` — REMOVE. Nothing exposes talent email/phone on any public
      surface (`views/portfolio/show.ejs`, `portfolio-pro.ejs`, and the PDF templates
      all carry no contact block). A privacy toggle that defaults to "on" and implies
      exposure that never happens is worse than no toggle.
- [x] `blockedAgencies` — REAL (`shared/lib/blocked-agencies.js`). Keep.

### Cookies
- [x] `analytics` — REAL, gates portfolio visitor tracking via the shared
      `pholio_consent` cookie. Keep.
- [x] `marketing` — REMOVE. The canonical consent contract (`shared/lib/consent.js`)
      has no marketing field at all; nothing reads the DB mirror.

### Membership
- [x] Invoices card — REMOVE. Server hardcoded `invoices: []`; the card promised
      "receipts appear here" and never could. Stripe's customer portal (already
      linked from "Manage billing") is the honest destination.

### Sessions — the trust failure (5 distinct root causes)
- [x] Full table scan: `listSignedInSessions` selected **every** session row in the
      database and filtered in JS. No WHERE clause.
- [x] Expired rows were listed as devices — it was a history log, not active devices.
- [x] `device: "Browser session"` / `location: "Current workspace"` were hardcoded
      literals. No user agent or IP was ever recorded, and the UI hardcoded a
      desktop `Monitor` icon — which is why an iPhone displayed as desktop.
- [x] In serverless, `cleanupInterval = 0` (`src/app.js`) means expired rows are
      **never** purged, so rows accumulate permanently.
- [x] Orphan churn: `PholioAuthBridge` re-syncs on every focus/visibilitychange.
      When `fetchAppSession()` doesn't come back authenticated it POSTs `/login`,
      which calls `req.session.regenerate()` — that destroys only the *current* sid.
      If the cookie wasn't presented (iOS Safari, cross-subdomain), the current sid
      is a fresh empty one, so the previously authenticated row is orphaned and a
      new one written. Every occurrence adds a permanent row → hundreds.

Fixes: record real device metadata at sign-in; one row per device per user
(fingerprint-scoped prune at login); list only live rows with a real WHERE clause;
purge this user's expired rows on read; derive last-active from the store's own
`expired` touch; correct per-device icons; add "sign out everywhere else".

### Auth identity
- [x] No `auth_provider` column existed — `normalizeOAuthUser` computed a provider at
      login and threw it away. Settings flattened everything into a disabled
      "Sign-in email" input.
- [x] Persist Firebase's authoritative `firebase.sign_in_provider` claim.
- [x] Show Google with real Google branding; suppress the password-reset row for
      Google accounts (a Firebase password reset on a Google-only account is
      misleading at best).

### Missing settings — considered and decided
- [x] Sign out everywhere else — ADDED (real need, now that sessions are truthful).
- [x] Connected sign-in method — ADDED.
- [ ] Metric/imperial unit preference — DELIBERATELY NOT ADDED. Comp cards and
      profile surfaces already print both (`shared/lib/stats-formatter.js`,
      `measurementConversions.js`), which is the industry convention. A toggle would
      be a preference over something already shown both ways.
- [ ] Two-factor — not addable honestly on the current Firebase setup; left out
      rather than stubbed.

## Review

Settings went from 9 sections to 8, and from 20 controls to 12 — every remaining
one traced to a consumer before it was kept.

Removed (no consumer, verified by grep across `src`, `views`, `client`): comp-card
layout, cover image, watermark, email digest rhythm, "Messages" toggle, "Product
notes", `showContact`, marketing cookie, the Invoices card, and the inert
erasure-request endpoint.

Added: real device identification, one live session row per device, "sign out
everywhere else", and first-class Google sign-in identity with password reset
suppressed for provider accounts.

A real bug surfaced while testing: `connect-session-knex` stores `expired` as an
ISO 8601 **string**, so the first version of the device query used
`where("expired", ">", someDate)` — which on SQLite compares a knex-serialised
integer against ISO text and silently matches nothing. The device list would have
come back empty and the expiry purge would have deleted nothing, in production as
well as in tests. `expiryCondition()` now mirrors the store's own dialect-aware
comparison.

Verification:
- 46 new tests pass — `session-device` (UA parsing incl. the iPhone regression),
  `session-registry` (real DB: scoping, expiry, collapse of 41 rows to 1 device,
  cross-account revoke refusal), `settings-contract` (end-to-end save → fresh read
  → repopulate, per-account scoping, and assertions pinning the removed settings
  so they cannot creep back).
- Client lint clean, production build succeeds.
- Migration applies and rolls back.
- Note: this repo's full Jest run has substantial pre-existing failures caused by
  parallel suites sharing one SQLite file (`dev.sqlite3`). Each suite implicated
  in that noise passes in isolation, before and after this change. One genuine
  issue of my own was found and fixed there: the new session tests originally ran
  a blanket `sessions` delete, which signed out other suites' fixture users
  mid-run; they now only ever touch rows they created.
...
## Goal
Fill Google and Instagram sign-in buttons on `/login` with each provider's native brand treatment (not outline/neutral chrome).

## Plan
- [x] Branch `cursor/login-oauth-native-branding-6e1e`
...
# AI Agent Operating Rule — 2026-07-24

## Plan
- [x] Draft durable, provider-agnostic operating rule (model routing, work split, tokens, quality, parallel coordination)
- [x] Add Cursor always-apply rule under `.cursor/rules/`
...
- Canonical: `docs/ai-agent-operating-rule.md`
- Cursor always-apply: `.cursor/rules/ai-agent-operating-rule.mdc`
- `CLAUDE.md` / `AGENTS.md` replace thin Model Routing with a global summary + parallel/token guidance; Subagent Strategy points at the rule
- `tasks/lessons.md` records the global rule and links the 2026-07-11 parallel-ownership lesson to §6
